...
class GoogleAuthError(RuntimeError):
    """Raised when Google cannot issue a usable Drive access token."""
...
def load_settings() -> GoogleSettings:
    """Load local desktop OAuth credentials without exposing them."""
...
def _wait_for_callback(port: int, expected_state: str, authorization_url: str) -> str:
    ...
    class CallbackHandler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802 - required handler name
            ...
            self.end_headers()
            self.wfile.write(b"<h1>Google Drive authorization complete.</h1><p>You can close this tab.</p>")
            completed.set()
    ...
    try:
        ...
        if thread.is_alive():
            raise GoogleAuthError("Timed out waiting for Google sign-in (5 minutes).")
        if result.get("error"):
...
def get_access_token(*, force_login: bool = False, token_path: Path = TOKEN_CACHE) -> str:
    """Return a Drive token, refreshing cached credentials or opening browser consent."""
    settings = load_settings()
    cached = _read_cache(token_path)
    ...
    if cached and not force_login and cached.get("refresh_token"):
        ...
        _write_cache(refreshed, token_path)
        return refreshed["access_token"]

    # Bind a temporary socket only to reserve a free loopback port. The callback
    # handler starts on that port immediately before the browser is opened.
