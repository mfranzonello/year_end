"""Run Google Drive to OneDrive migration without mounted cloud drives."""
...
def migrate(project_year: int, dry_run: bool = True) -> None:
    ...
    try:
        ...
        ui.add_update(
            f"Starting Google Drive cloud migration for {project_year} ({mode})."
        )
        ...
        ui.add_update("Google Drive cloud migration complete.")
    finally:
        engine.dispose()


def main() -> None:
    """Parse cloud-migration arguments and run the selected year."""
    parser = argparse.ArgumentParser(
        description="Stream missing Google Drive videos into OneDrive.",
    )
    parser.add_argument(
        "--year", type=int, default=date.today().year,
        help="Project year to migrate; defaults to the current year.",
    )
    ...
    migrate(arguments.year, dry_run=not arguments.apply)
