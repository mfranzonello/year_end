...
from database.db import get_engine, Engine
...
from charting.cloudy import configure_cloud, CloudConfig
...
with cols[0]:
    ...
    person_id:UUID = st.selectbox('Person to Center', persons['member_id'],
                                  ...
                                  index = None, #int(persons[persons['member_id'] == founder_id].index[0]),
                                  width=400)
...
if person_id:
    ...
    with cols[3]:
        ...
        with st.container():
            if not use_images:
                with_flag = st.checkbox('Include Origin Flag', value=False,
                                        help='Show the flag of the region of member birth')
            else:
                with_flag = False
...
if person_id:
    ...
    def create_tree_chart(engine:Engine, tree_data:DataFrame, cloud:CloudConfig,
                          use_images:bool, generation_limit:int, with_flag:bool):
        return tree_chart(engine, tree_data, cloud=cloud,
                          use_images=use_images, generation_limit=generation_limit, with_flag=with_flag)

    if len(tree_data):
        # graph with nodes and edges
        with st.spinner('Building tree...', show_time=True):
            graph = create_tree_chart(engine, tree_data, cloud, use_images, generation_limit, with_flag)
